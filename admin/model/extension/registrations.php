...
class ModelExtensionRegistrations extends Model {
	public function addRegistrations($data) {
		$query  = "INSERT INTO " . DB_PREFIX . "registrations SET customer_id = '" . $this->db->escape($data['customer_id']) . "', store_id = '" . $this->db->escape($data['store_id']) . "', date_added = NOW(), status = '" . (int)$data['status'] . "'";
		...
		$query .= " ,address_1 ='".$this->db->escape($data['address'])."'"; 
		...
		$query .= " ,class_id ='".$this->db->escape(explode(',',$data['class_id']))."'"; 
		$query .= " ,approved ='".$this->db->escape($data['approved'])."'"; 
		...
		if($last_id > 0)
		{
			$this->updateClassRegData($last_id,$data['class_id'],'ADD');
		}
		return $last_id;
	}
	
	public function editRegistrations($reg_id, $data) {
		$query  = "UPDATE " . DB_PREFIX . "registrations SET customer_id = '" . $this->db->escape($data['customer_id']) . "', store_id = '" . $this->db->escape($data['store_id']) . "', date_added = NOW(), status = '" . (int)$data['status'] . "'";
		$query .= " ,firstname ='".$this->db->escape($data['firstname'])."'"; 
		$query .= " ,lastname ='".$this->db->escape($data['lastname'])."'"; 
		$query .= " ,email ='".$this->db->escape($data['email'])."'"; 
		$query .= " ,telephone ='".$this->db->escape($data['telephone'])."'"; 
		...
		$query .= " ,zipcode ='".$this->db->escape($data['zipcode'])."'"; 
		$query .= " ,newsletter ='".$this->db->escape($data['newsletter'])."'"; 
		$query .= " ,class_id ='".$this->db->escape(join(',',$data['class_id']))."'"; 
		$query .= " ,job_title ='".$this->db->escape($data['job_title'])."'"; 
		$query .= " ,experience ='".$this->db->escape($data['experience'])."'"; 
		$query .= " ,location ='".$this->db->escape($data['location'])."'"; 
		$query .= " ,date_training ='".$this->db->escape($data['date_training'])."'";  
		$query .= " ,returning_student ='".$this->db->escape($data['returning_student'])."'"; 
		$query .= " ,message ='".$this->db->escape($data['message'])."'"; 
		$query .= " ,deposit ='".$this->db->escape($data['deposit'])."'"; 
		$query .= " ,approved ='".$this->db->escape($data['approved'])."' WHERE registration_id = '" . (int)$reg_id . "'";
		
		$result = $this->db->query($query);	
		if($result){
			$this->updateClassRegData($reg_id,$data['class_id'],'update');
		}
		return $result;		
	}
	...
	public function getAllRegistrations($data) {
		$sql = "SELECT *,r.registration_id as reg_id,r.status as r_status, CONCAT(r.firstname, ' ', r.lastname) AS name, cg.`title` as classname, rcn.amount_fees, r.deposit  FROM " . DB_PREFIX . "registrations r  LEFT JOIN " . DB_PREFIX . "registration_classname rc ON (r.registration_id=rc.registration_id) LEFT JOIN " . DB_PREFIX . "news_description cg ON (rc.class_id=cg.news_id) LEFT JOIN " . DB_PREFIX . "news rcn ON (rcn.news_id=cg.news_id)  WHERE 1=1 ";
		...
		if (!empty($data['filter_date_added'])) {
			$implode[] = "DATE(r.date_training) = DATE('" . $this->db->escape($data['filter_date_added']) . "')";
		}
		...
		if (isset($data['sort']) && in_array($data['sort'], $sort_data)) {
			...
			$sql .= " ORDER BY CONCAT(r.firstname, ' ', r.lastname)";
		}
	...
	public function deleteRegistrations($reg_id) {
		...
		$this->db->query("DELETE FROM " . DB_PREFIX . "registration_classname WHERE registration_id = '" . (int)$reg_id . "'");
	}
