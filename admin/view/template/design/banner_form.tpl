          ...
          <table id="images" class="table table-striped table-bordered table-hover">
            ...
            <tbody>
              ...
              <tr id="image-row<?php echo $image_row; ?>">
                ...
                <td class="text-left"><button type="button" onclick="$('#image-row<?php echo $image_row; ?>').remove();$('#image-row<?php echo $image_row+1; ?>').remove();" data-toggle="tooltip" title="<?php echo $button_remove; ?>" class="btn btn-danger"><i class="fa fa-minus-circle"></i></button></td>
              </tr>			 
			  <tr id="image-row<?php echo $image_row + 1; ?>">
			  <td colspan="4">
			   <?php foreach ($languages as $language) { ?>			  	  
				  	  <div class="input-group pull-left"><span class="input-group-addon"><img src="view/image/flags/<?php echo $language['image']; ?>" title="<?php echo $language['name']; ?>" /> </span>
				  	 <textarea name="banner_image[<?php echo $image_row; ?>][banner_image_description][<?php echo $language['language_id']; ?>][description]" id="input-description<?php echo $image_row; ?>" rows="4" class="form-control" placeholder="Description Banner..."><?php echo isset($banner_image['banner_image_description'][$language['language_id']]) ? $banner_image['banner_image_description'][$language['language_id']]['description'] : ''; ?></textarea></div>		 
				  <?php } ?>
				   </td>
			  </tr>			  
              <?php $image_row+=2; ?>
...
function addImage() {
	...
	html += '  <td class="text-left"><button type="button" onclick="$(\'#image-row' + image_row  + '\').remove();$(\'#image-row' + (image_row +1)  + '\').remove();" data-toggle="tooltip" title="<?php echo $button_remove; ?>" class="btn btn-danger"><i class="fa fa-minus-circle"></i></button></td>';
	html += '</tr>';
	html += '<tr id="image-row'+(image_row+1)+'">';
	html += '   <td colspan="4">';
			  <?php foreach ($languages as $language) { ?>
	...
	html += '	 		<textarea name="banner_image['+image_row+'][banner_image_description][<?php echo $language['language_id']; ?>][description]" id="input-description' + image_row + '" rows="4" class="form-control" placeholder="Description Banner..."></textarea> </div>';
					<?php } ?>
	html += '   </td>';
	html += '  </tr>';
