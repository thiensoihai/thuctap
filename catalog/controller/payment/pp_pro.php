...
class ControllerPaymentPPPro extends Controller {
	...
	public function send() {
		...
		$name_on_card = $this->request->post['name_on_card'];
		if(empty($name_on_card))
		{
			$json['error'] = 'Please enter your name on card';
		}
			else
		{
			$name_on_card = explode(' ',$this->request->post['name_on_card']);
			$firtname = $name_on_card[0];
			$lastname = [];
			foreach($name_on_card as $key=>$value)
			{
				if($key > 0)
				$lastname[] = $value;
			}
			...
			$request .= '&FIRSTNAME=' . urlencode($firtname);
