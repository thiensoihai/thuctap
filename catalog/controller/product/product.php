...
class ControllerProductProduct extends Controller {
	...
	public function index() {
		...
		if ($product_info) {
			...
			$this->document->addScript('catalog/view/javascript/ilashmax/rate/js/star-rating.min.js');
			$this->document->addStyle('catalog/view/javascript/ilashmax/rate/css/star-rating.css');
			
			$data['heading_title'] = $this->model_catalog_product->cutString($product_info['name'],6);
			...
			if ($product_info['image']) {
				...
				$data['small_popup'] = $this->model_tool_image->resize($product_info['image'], $this->config->get('config_image_category_width'), $this->config->get('config_image_category_height'));
			} else {
				$data['popup'] = '';
			}
			...
			foreach ($results as $result) 
			{
				$data['images'][] = array(
					...
					'small' => $this->model_tool_image->resize($result['image'], $this->config->get('config_image_category_width'), $this->config->get('config_image_category_height')),
			...
			if (($this->config->get('config_customer_price') && $this->customer->isLogged()) || !$this->config->get('config_customer_price')) {
				$sub_price = $this->tax->calculate($product_info['price'], $product_info['tax_class_id'], $this->config->get('config_tax'));
				$data['price'] = $this->currency->format($sub_price);
			} else {
				$data['price'] = false;
			}
			$sub_special=0;
			if ((float)$product_info['special']) {
				$sub_special = $this->tax->calculate($product_info['special'], $product_info['tax_class_id'], $this->config->get('config_tax'));
			...
			foreach ($this->model_catalog_product->getProductOptions($this->request->get['product_id']) as $option) {
				...
				$discount = isset($sub_special) ? round(($sub_special * 100)/$sub_price,2) : 0;
			...
			foreach ($results as $result) {
				...
				foreach ($img_results as $img) {
					$sub_images[] = $this->model_tool_image->resize($img['image'], $this->config->get('config_image_related_width'), $this->config->get('config_image_related_height'));
				}
