...
$.sharer = {
	...
	"options": {
		"networks": ["facebook", "twitter", "linkedin", "tumblr", "gplus", "reddit", "pinterest", "stumbleupon", "taringa"],
		"template": $('<a class="sharer-icon"></a>'),
		"class": "social-icon si-colored si-%network.id%",
		"label": "Share on %network.name%",
		"html": '<i class="icon-%network.id%"></i><i class="icon-%network.id%"></i>',
		"title": null,
		"description": null,
		"url": null
	}
	...
	return this.each(function () {
		...
		for (var i = 0; i < options["networks"].length; i++) {
			...
			button
				...
				.html((options["html"].replace("%network.id%", network)).replace("%network.id%", network))
				.click(function () {
