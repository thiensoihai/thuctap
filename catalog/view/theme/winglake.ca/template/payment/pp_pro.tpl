<form class="form-horizontal">
  <fieldset id="payment">
	...
	<div class="form-group required">
      <label class="col-sm-2 control-label" for="input-name_on_card"><?php echo $entry_cc_name; ?></label>
      <div class="col-sm-10">
        <input type="text" name="name_on_card" value="" placeholder="<?php echo $entry_cc_name; ?>" id="input-name_on_card" class="form-control" />
      </div>
    </div>
    <div class="form-group required">
    ...
    <div class="form-group" id="credit_cc_issue" style="display: none;">
...
$('#input-cc-type').change(function(event){
	var value_data = $(this).val();
	if(value_data==='SWITCH' || value_data==='SOLO'){
		$('#credit_cc_issue').show();
	}else{
		$('#credit_cc_issue').hide();
	}
});
