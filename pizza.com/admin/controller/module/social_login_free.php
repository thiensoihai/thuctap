...
class ControllerModuleSocialLoginFree extends Controller {
	private $id = 'social_login_free';
	private $route = 'module/social_login_free';
	...
	public function index() {
       ...
       if ($this->model_setting_setting->getSetting($this->id))
		...
		$data['setting'] = $setting['social_login_free_module']['setting'];
	...
	protected function validate() {
		if (!$this->user->hasPermission('modify', 'module/social_login_free')) {
	...
	public function get_light_or_full_version(){
		$full = DIR_SYSTEM . "config/social_login_free.php";
		$light = DIR_SYSTEM . "config/social_login_free_lite.php"; 
		if (file_exists($full)) { 
			return 'social_login_free';
		} elseif (file_exists($light)) {
			return 'social_login_free_lite';
		}else{
			return false;
		}
