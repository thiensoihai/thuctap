<?php
class ControllerModuleSocialLoginFree extends Controller {
		
	public function index($setting) {
		//echo $setting;
		if(empty($setting) ){
			$setting = $this->config->get( 'social_login_free_module');
		...
		$this->session->data['social_login_free'] = $setting;
		...
		$this->language->load('module/social_login_free');
		$this->document->addStyle('catalog/view/theme/default/stylesheet/social_login_free/styles.css');
		$this->document->addScript('catalog/view/javascript/social_login_free/spin.min.js');
		...
		$this->session->data['social_login_free']['return_url'] = $this->getCurrentUrl();

		
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/module/social_login_free.tpl')) {
			return $this->load->view($this->config->get('config_template') . '/template/module/social_login_free.tpl', $data);
		} else {
			return $this->load->view('default/template/module/social_login_free.tpl', $data);
		}
	}

	public function hybridauth(){

		$this->load->model('social_login_free/hybridauth_model');
		return $this->model_social_login_free_hybridauth_model->process();
...
*/
	public function register($provider,$profile){
		...
		if($profile){
			...
			if($customer){
				...
				$this->response->redirect(urldecode($this->session->data['social_login_free']['return_url']));
			}else{
			...
			//checking if email sent
				...
				if(!$popup){
					...
					if(!$customer){
						...
						$this->response->redirect($this->return_to_page($setting['return_page'], $this->session->data['social_login_free']['return_url']));
					}else{
						...
						$this->response->redirect($this->session->data['social_login_free']['return_url']);
					}
				}else{
						$this->language->load('module/social_login_free');
						$sort_order = array(); 
						...
						if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/social_login_free/email.tpl')) {
							 $this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/social_login_free/email.tpl', $data));
						} else {
							 $this->response->setOutput($this->load->view('default/template/social_login_free/email.tpl', $data));
						}
	...
	public function create_with_email(){
		...
		if(!$error){
				...
				$this->provider_edit_customer($this->customer->getId(), $data);
				$json['url'] = $this->return_to_page($setting['return_page'], $this->session->data['social_login_free']['return_url']);
		}else{
			$json['error'] = $error; 
		}
