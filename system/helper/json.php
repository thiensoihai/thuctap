<?php
if (!function_exists('create_menu_item')) 
{
	function create_menu_item($data)
	{		
		$html = '<li><a href="'.$data['href'].'"><div>'.$data['name'].'</div></a>';
		...
		{			
			...
			foreach($data['children'] as $key=>$value)
			{
				if(count($value['children']) > 0)
					$html .= create_menu_item($value);
				else
				$html .= '<li><a href="'.$value['href'].'"><div>'.$value['name'].'</div></a></li>';
			}
			$html .= '</ul>';
		}
		$html.='</li>';
